...
fonctionne très mal, ne pas prendre en compte : pb de la méthode usuelle en variable latente est que les variables catégorielles non ordonnées ne sont pas implémentables en tant que tel, obligé de les prendre de manière ordonnées. cela a a voir avec l'indépendance conditionnelle aux variables latentes des variables observées 
...
\section{Chômage v santé}

But: regarder si les retraités bougent d'une analyse à l'autre. Autrement dit essayer de séparer le soutien à l'ensemble de la protection sociale et aux branches respectives.
...
Implémenter les résultats sur latex pour qu'ils soient lisibles
```{r}
library(knitr)
